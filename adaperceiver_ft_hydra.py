...
class Model(L.LightningModule):
    ...
    def compute_losses(self, pred_list, label, loss_weights):
        supervised_loss = (
    ...
    def training_step(self, batch, batch_idx):
        ...
        output_list, inter_list, (out_feat_list, intermediate_feat_outs) = self.model.forward(
            x,
        ...
        supervised_loss = 0.0
        # Token Loss
        token_supervised_loss = self.compute_losses(
            output_list,
            label,
        ...
        depth_sup_loss = 0.0
        if self.depth_loss:
            depth_sup_loss = self.compute_losses(
                inter_list,
                label,
        ...
        self.log(
            "train/token_sup_loss",
            token_supervised_loss,
            on_epoch=True,
            prog_bar=True,
            logger=True,
        )
        self.log(
            "train/depth_sup_loss",
            depth_sup_loss,
            on_epoch=True,
            prog_bar=True,
            logger=True,
    ...
    def validation_step(self, batch, batch_idx):
        ...
        supervised_loss = self.compute_losses(
            output_list,
            label,
            self.token_loss_weights if self.token_loss else [1.0],
        )
        ...
        # ema model eval
        if self.ema_model:
            ema_output_list, ema_inter_list, (ema_out_feat_list, ema_intermediate_feat_outs) = self.ema_model.forward(
                x,
                num_tokens=None,
                mat_dim=None,
                token_loss=self.token_loss,
            ...
            ema_supervised_loss = self.compute_losses(
                ema_output_list,
                label,
                self.token_loss_weights if self.token_loss else [1.0],
            )
            if self.depth_loss:
                ema_depth_sup_loss = self.compute_losses(
                    ema_inter_list,
                    label,
                    self.depth_loss_weights,
                )
    ...
    def configure_optimizers(self):
        ...
        if optimizer_cfg.output_adapters:
            ...
            parameters += self.model.output_latents.parameters()
            parameters += list(self.model.write_head.parameters())
            for name, param in self.model.named_parameters():
                if "output_adapter" in name or "output_latents" in name or "write_head" in name:
...
@hydra.main(version_base=None, config_path="config/supervised")
def main(cfg: DictConfig):
    ...
    if cfg.model.model_checkpoint:
        ...
        state_dict['process_latents.token'] = state_dict['process_latents.token'].squeeze()
        state_dict['patch_embed.pos_embed'] = state_dict['patch_embed.pos_embed'] #.squeeze()
        state_dict['output_latents.token'] = state_dict['output_latents.token']

...
def eval(cfg, model: AdaptiveDepth, loader, device):
    @torch.inference_mode()
    def _eval_config(num_tokens, mat_dim, depth):
        ...
        with torch.autocast(device_type=device, dtype=torch.bfloat16):
            for batch in tqdm(
                ...
                preds1, preds2 = output[-1].chunk(2, dim=0)
                preds2 = preds2.flip(-1)
                preds = (preds1 + preds2) / 2.0  # Average
                metric(preds, label)
...
@hydra.main(version_base=None, config_path="config/evaluation/depth")

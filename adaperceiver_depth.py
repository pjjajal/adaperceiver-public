...
from modules.depth.depther import AdaptiveDepth
...
class Model(L.LightningModule):
    ...
    def compute_loss(self, pred_list, label, loss_weights):
        sig_loss = sum(
            [
                w * self.sig_loss(output, label)
                for w, output in zip(loss_weights, pred_list)
            ]
        )
        sig_loss = sig_loss / sum(loss_weights)
        grad_loss = sum(
            [
                w * self.grad_loss(output, label)
                for w, output in zip(loss_weights, pred_list)
            ]
        )
        grad_loss = grad_loss / sum(loss_weights)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        output_list, inter_list = self.model.forward(
            x,
            num_tokens=self.token_grans[-1],
            mat_dim=None,
            token_loss=self.token_loss,
            layer_loss=self.depth_loss,
            freq_cis=freq_cis,
        )
        # Separate flipped outputs
        output_list_flipped = []
        for output in output_list:
            preds1, preds2 = torch.chunk(output, 2, dim=0)
            preds2 = preds2.flip(-1)
            preds = (preds1 + preds2) / 2.0
            output_list_flipped.append(preds)
        output_list = output_list_flipped
        
        inter_list_flipped = []
        for inter in inter_list:
            preds1, preds2 = torch.chunk(inter, 2, dim=0)
            preds2 = preds2.flip(-1)
            preds = (preds1 + preds2) / 2.0
            inter_list_flipped.append(preds)
        inter_list = inter_list_flipped
        ...
        pred_depths = output_list[-1]
        self.rmse(pred_depths, label)
        self.log(
            "val/test_loss",
            loss,
            on_epoch=True,
        ...
        if self.ema_model:
            ...
            # Separate flipped outputs
            ema_output_list_flipped = []
            for output in ema_output_list:
                preds1, preds2 = torch.chunk(output, 2, dim=0)
                preds2 = preds2.flip(-1)
                preds = (preds1 + preds2) / 2.0
                ema_output_list_flipped.append(preds)
            ema_output_list = ema_output_list_flipped

            ema_inter_list_flipped = []
            for inter in ema_inter_list:
                preds1, preds2 = torch.chunk(inter, 2, dim=0)
                preds2 = preds2.flip(-1)
                preds = (preds1 + preds2) / 2.0
                ema_inter_list_flipped.append(preds)
            ema_inter_list = ema_inter_list_flipped
            ...
            pred_depths = ema_output_list[-1]
            self.rmse_ema(pred_depths, label)
            self.log(
                "val/ema_test_loss",
                ema_loss,
                on_epoch=True,
...
@hydra.main(version_base=None, config_path="config/depth")
def main(cfg: DictConfig):
    ...
    seg_model = AdaptiveDepth(
        backbone,
        num_bins=cfg.dataset.bins,
        head_drop=cfg.model.encoder_config.depth_head_drop,

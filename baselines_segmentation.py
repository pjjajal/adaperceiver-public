...
class Model(L.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        preds = self.model(x)
        loss = self.criterion(preds, label)
        self.miou.add_batch(predictions=preds.argmax(dim=1), references=label)
        ...
        if self.ema_model:
            ...
            ema_loss = self.criterion(preds, label)
            self.miou_ema.add_batch(predictions=preds.argmax(dim=1), references=label)
            self.log(
                "val/ema_test_loss",
                ema_loss,
                on_epoch=True,
    ...
    def process_gathered_metrics(self, metric_keys, gathered_metrics):
        all_metrics = {}
...
@hydra.main(version_base=None, config_path="config/segmentation/baselines")

...
class BaselineSeg(nn.Module):
    def __init__(
        ...
        in_channels = backbone.embed_dim * 2 if cat_cls else backbone.embed_dim
        self.head = LinearSegHead(in_channels, num_classes, head_drop)
        self.mlp = Mlp(backbone.embed_dim, backbone.embed_dim, drop=head_drop) if use_mlp else nn.Identity()
    ...
    def forward(self, img: torch.Tensor) -> torch.Tensor:
        ...
        if self.cat_cls:
            cls_token = self.backbone.pool(feats).unsqueeze(1).expand(-1, patch_feats.shape[1], -1)
        ...
        return self.head(img, patch_feats)


class AdaptiveSeg(nn.Module):
    def __init__(
        self,
        ...
        in_channels = backbone.output_feat_dim * 2 if cat_cls else backbone.output_feat_dim
        self.head = LinearSegHead(in_channels, num_classes, head_drop)
    ...
    def forward(
        ...
        **kwargs
        ...
        if layer_loss:
            intermediate_outs = [self.forward_head(img, out, H, W) for out in intermediate_outs]

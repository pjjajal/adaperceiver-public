...
from modules.layers.latents import PatchEmbedOutputLatents
...
class DensePredictionAdaPerceiver(AdaPerceiver):
    def __init__(self, config: AdaPercevierConfig, dense_config: DensePredictionConfig):
        super().__init__(config)
        self.dense_config = dense_config
        self.use_embed_ffn = dense_config.use_embed_ffn
        self.use_output_ffn = dense_config.use_output_ffn
        self.img_size = dense_config.img_size
        self.in_channels = dense_config.in_channels
        self.patch_size = dense_config.patch_size
        self.use_fourier = dense_config.use_fourier
        self.fourier_bands = dense_config.fourier_bands
        self.output_feat_dim = dense_config.output_feat_dim
        ...
        if dense_config.adapter_type == "patch_embed":
            self.patch_embed = PatchEmbedAdapter(
                ...
                use_fourier=self.dense_config.use_fourier,
                fourier_bands=self.dense_config.fourier_bands,
            )
        elif dense_config.adapter_type == "conv":
            self.patch_embed = ConvAdapter(
                img_size=self.img_size,
                model_name=self.dense_config.conv_adapter_model,
                embed_dim=self.embed_dim,
                use_ffn=self.use_embed_ffn,
                ffn_layer=ffn_layer,
                ffn_ratio=self.config.ffn_ratio,
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.training:
            output_latents = self.output_latents.forward(
                patches.clone()
            )
        else:
        ...
        return out_list, intermediate_outs, (None, None)

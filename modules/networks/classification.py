...
class ClassificationAdaPerceiver(AdaPerceiver):
    def __init__(self, config: AdaPercevierConfig, cls_config: ClassificationConfig):
        super().__init__(config)
        self.cls_config = cls_config
        self.num_classes = cls_config.num_classes
        self.use_embed_ffn = cls_config.use_embed_ffn
        self.use_output_ffn = cls_config.use_output_ffn
        self.img_size = cls_config.img_size
        self.in_channels = cls_config.in_channels
        self.patch_size = cls_config.patch_size
        self.use_fourier = cls_config.use_fourier
        self.fourier_bands = cls_config.fourier_bands
        ...
        if cls_config.adapter_type == "patch_embed":
            self.patch_embed = PatchEmbedAdapter(
                ...
                use_fourier=self.cls_config.use_fourier,
                fourier_bands=self.cls_config.fourier_bands,
            )
        elif cls_config.adapter_type == "conv":
            self.patch_embed = ConvAdapter(
                img_size=self.img_size,
                model_name=self.cls_config.conv_adapter_model,
                embed_dim=self.embed_dim,
                use_ffn=self.use_embed_ffn,
                ffn_layer=ffn_layer,
                ffn_ratio=self.config.ffn_ratio,
            ...
            if depth_tau is not None:
                ...
                if avg_conf >= depth_tau:
                    if depth_counter is not None:
                        depth_counter['current_depth'] = i + 1
                        depth_counter[i + 1] += 1
                    break
        return process_latents
    ...
    def forward(
        ...
        **kwargs
        ...
        process_latents = self.read(x, process_latents, freq_cis)
        ...
        if layer_loss:
            ...
            process_latents = self.forward_block_conf(
                ...
                mat_dim=mat_dim,
                depth_tau=depth_tau,
                depth_counter=kwargs.get("depth_counter", None),
            )
        else:
        ...
        if layer_loss:
            for intermediate in intermediates:
                ...
                write_out_tokens = (
                    random.choice(self.mask_token_grans)
